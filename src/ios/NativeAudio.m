...
- (void) preloadComplex:(CDVInvokedUrlCommand *)command
{
    ...
    [self.commandDelegate runInBackground:^{
        if (existingReference == nil) {
            ...
            if ([assetPath rangeOfString:@"file:"].location != NSNotFound) {
                NSString* fileAssetPath  = [assetPath stringByReplacingOccurrencesOfString:@"file://" withString:@""];
                path = fileAssetPath;
            }
...
- (void) playSprite:(CDVInvokedUrlCommand *)command
{
    NSString *callbackId = command.callbackId;
    NSArray* arguments = command.arguments;
    NSString *audioID = [arguments objectAtIndex:0];
    ...
    if ( [arguments count] > 1 ) {
        ...
        stime = [arguments objectAtIndex:1];
        ptime = [arguments objectAtIndex:2];

        if([stime isEqual:nil]) {
        ...
        if (audioMapping) {
            ...
            if (asset != nil){
                if ([asset isKindOfClass:[NativeAudioAsset class]]) {
                    ...
                    [_asset playSprite:stime playTime:ptime];
